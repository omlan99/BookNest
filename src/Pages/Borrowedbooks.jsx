...
const Borrowedbooks = () => {
  ...
  return (
    <div className=" mt-[100px] mb-20">
      {
        borrowedBooks.length > 0 ?  <><div className="grid lg:grid-cols-4 md:grid-cols-3 gap-6 px-5">
        {borrowedBooks.map((borrowedBook, index) => (
          <div key={index} className="card card-compact bg-base-100 shadow-md">
            ...
            <div className="card-body flex-grow">
              <h2 className="card-title text-lg">{borrowedBook.bookName}!</h2>
              ...
              <p className="text-left ">{borrowedBook.category}</p>
                ...
                <p className="py-2">Returnning time {borrowedBook.returnDate }</p>

                </div>
              <div className="card-actions justify-end mt-2">
                {/* <Link to={`/book_details/${book._id}`}> */}
                <button onClick={() => handleReturn(borrowedBook._id, borrowedBook.bookId)} className="btn btn-primary">Return</button>
              {/* </Link> */}
              </div>
            </div>
          </div>
        ))}
      </div></>: <><div className="w-full  min-h-screen flex justify-center items-center"><p className="text-2xl font-bold">You did not borrowed any books yet</p></div></>
      }
    </div>
  );
};
